...
class MapLayers extends StatelessWidget {
    ...
    return Stack(
      children: [
        ...
        if (currentGeofencePoints.isNotEmpty)
          MarkerLayer(markers: _buildGeofenceDrawingMarkers()),
        ...
        if (showHistoryPoints)
          PolylineLayer(polylines: _buildDevicePolylines()),

        // Historical device markers - ONLY SHOW IF showHistoryPoints IS TRUE
        if (showHistoryPoints)
          MarkerLayer(markers: _buildHistoricalDeviceMarkers()),

        // Current user location accuracy circle
        if (currentPosition != null)
          CircleLayer(circles: _buildUserLocationCircle()),

        // Current user location marker
        if (currentPosition != null)
          MarkerLayer(markers: _buildUserLocationMarkers()),

        // Latest device location markers (ALWAYS VISIBLE)
  ...
  List<Polygon> _buildGeofencePolygons() {
    return [
      // All existing geofences display in GREEN
      ...geofences.map(
        (geofence) => Polygon(
      ...
      if (currentGeofencePoints.length >= 3)
        Polygon(
          ...
          color: Colors.green.withOpacity(0.2),
          borderColor:
              isDragging ? Colors.green.shade700 : Colors.green.shade600,
  ...
  List<Polyline> _buildGeofencePolylines() {
    return [
      Polyline(
        ...
        color: (isDragging ? Colors.green.shade700 : Colors.green.shade600)
            .withOpacity(0.8),
      ),
    ];
  }
        ...
        Marker(
          ...
          builder:
              (context) => GestureDetector(
                onTap: () {
                  onDeviceSelected(
                    selectedDeviceId == deviceId ? null : deviceId,
                  );
                },
                child: Container(
                  decoration: BoxDecoration(
