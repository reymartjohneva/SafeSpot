...
class ChildInfoService {
  ...
  static Future<ChildInfo> upsertChildInfo(ChildInfo childInfo) async {
    ...
    try {
      ...
      final data = {
        ...
        'avatar_url': childInfo.avatarUrl,
      ...
      if (existing != null) {
        ...
        final response =
            await _supabase.from('child_info').insert(data).select().single();
  ...
  static Future<void> deleteChildInfo(String deviceId) async {
    ...
    try {
      ...
      await _supabase.from('child_info').delete().eq('device_id', deviceId);
    } catch (e) {
